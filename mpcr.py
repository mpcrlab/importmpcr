...
Original file is located at
    https://colab.research.google.com/drive/1nVCQRjhEyc_Hk3eTH89humUWQA3iP3jp
"""
...
def load_fits_make_jpeg(folder):
  
    img_size=(512,512)
  
    num_crops = 10

    for filename1,filename2,filename3 in zip(sorted(os.listdir(folder+'/new/')),sorted(os.listdir(folder+'/ref/')),sorted(os.listdir(folder+'/diff/'))):
      
        print(filename1,filename2,filename3)

        img_new  = fits.open(folder + 'new/' + filename1)[0].data
        img_ref  = fits.open(folder + 'ref/' + filename2)[0].data
        img_diff = fits.open(folder + 'diff/' + filename3)[0].data
                

        img_new_w = view_as_windows(img_new,img_size)
        img_ref_w = view_as_windows(img_ref,img_size)
        img_diff_w = view_as_windows(img_diff,img_size)
        ...
        for i in range(num_crops):
          ...
          A[:,:,0] = img_new_w[x,y,:,:]
          A[:,:,1] = img_ref_w[x,y,:,:]
          A[:,:,2] = img_new_w[x,y,:,:] - img_ref_w[x,y,:,:]
...
#           C = np.concatenate((A,B),axis=1)
          ...
          imageio.imwrite('/content/drive/Team Drives/sky_oddity_engine/data/ML_Data/test/a/DeepSpace'+ r + '.jpg', A)
          imageio.imwrite('/content/drive/Team Drives/sky_oddity_engine/data/ML_Data/test/b/DeepSpace'+ r + '.jpg', B)

        
#           return(C)

"""##Mount Google Team Drive and List Contents"""

drive.mount('/content/drive')

os.listdir('/content/drive/My Drive/DeepSpaceData')

os.listdir('/content/drive/Team Drives/sky_oddity_engine/data/full_frame')

folder = '/content/drive/Team Drives/sky_oddity_engine/data/full_frame/'

# load_fits_make_jpeg(folder)

from PIL import Image

# C = load_fits_make_jpeg(folder)
